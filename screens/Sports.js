...
export default class Sports extends React.Component {
  ...
  componentDidMount() {
    db.collection("questions").where('category', 'array-contains', 'Sports').onSnapshot((querySnapshot) => {
      var ques = [];
  ...
  doRefresh = () => {
    ...
    db.collection("questions").where('category', 'array-contains', 'Sports').get().then((queries) => {
      var ques = [];
      queries.forEach((doc) => {
          ques.push(doc.data());
      });
      ...
      await db.collection('users').doc(user.uid).get().then((doc) => {
        ...
        for(i=0; i<doc.data().question_answered.length; i++){
          if(doc.data().question_answered[i].question === the_slug){
            goDetail=false;
            navigate('QuestionResult', { from_where: 'Sports', question: the_question, your_vote: doc.data().question_answered[i].answer})
          }
        }
        if(i===doc.data().question_answered.length && goDetail){
          navigate('QuestionDetail', { from_where: 'Sports', question: the_question });
        }
      })
    }else{
      navigate('QuestionDetail', { from_where: 'Sports', question: the_question });
